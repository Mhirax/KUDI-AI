...
export class SubmitNinVerificationHandler implements ICommandHandler<
  SubmitNinVerificationCommand,
...
> {
  ...
  async execute(command: SubmitNinVerificationCommand): Promise<KycStatusResponseDto> {
    ...
    if (!result.matched) {
      profile.recordVerificationFailed(
        VerificationType.NIN,
        'Provider-returned name did not match',
      );
