...
// Password rule from backend: min 12 chars, upper, lower, number, special char.
...
export default function Signup() {
  ...
  function validate() {
    ...
    if (!PASSWORD_RULE.test(password)) {
      e.password = 'Min 12 characters, with uppercase, lowercase, number and special character';
    }
    setErrors(e);
          ...
          {password && (
            <p className={'signup__strength ' + passwordStrength}>
              {passwordStrength === 'strong'
                ? '✓ Strong password'
                : 'Needs: 12+ chars, upper, lower, number, special char'}
            </p>
          )}
